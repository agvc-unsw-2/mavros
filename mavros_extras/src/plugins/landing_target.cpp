...
public:
	...
	void initialize(UAS &uas_)
	{
		...
		if (listen_tf) {	// Listen to transform
			...
			land_target_sub = nh.subscribe("raw", 10, &LandingTargetPlugin::landtarget_cb, this, ros::TransportHints().tcpNoDelay());
		}
		else {			// Subscribe to PoseStamped msg
			pose_sub = nh.subscribe("pose", 10, &LandingTargetPlugin::pose_cb, this, ros::TransportHints().tcpNoDelay());
		}
	}
